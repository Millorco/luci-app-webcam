...
echo "Download ultima release (tar.gz) dal repository GitHub..."
rm -rf "$TMP_DIR" "$TAR_FILE"

# Scarica la release come tar.gz
if command -v wget &> /dev/null; then
    wget -O "$TAR_FILE" "https://github.com/Millorco/luci-app-webcam/releases/latest/download/luci-app-webcam.tar.gz"
else
    curl -L -o "$TAR_FILE" "https://github.com/Millorco/luci-app-webcam/releases/latest/download/luci-app-webcam.tar.gz"
fi

if [ $? -ne 0 ]; then
    echo "ERRORE: Impossibile scaricare la release"
    exit 1
fi
...
# Trova la directory estratta (solitamente luci-app-webcam-<versione>)
